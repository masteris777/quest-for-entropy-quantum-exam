...
**The underlying limitation is architectural.** Model Two is a ONE-SYSTEM machine: a single three-component state, one observer, one fold. The pair tested here was assembled from that machinery rather than designed in from the start, and in that assembly each wing's fold sees only its own setting — which is precisely the condition that makes a model Bell-local. Reproducing entanglement is therefore a REDESIGN, not a rerun, and it is parked for a later stage of the programme.
